...
bloom_sum <- bloomington %>% group_by(departments) %>% 
                summarise(`min` = min(salaries),
                          ...
                          `max` = max(salaries))
div(style='height=1500;overflow-y: scroll', 
  datatable(bloom_sum) %>% 
...
div(style='height=1500;overflow-y: scroll', 
  datatable(bloom_sum) %>% 
  formatCurrency(c('min', '25%', 'mean', 'median', '75%', 'max'), digits=0)     )    
```
...
### SQL for the Join {data-height=1000}
```
select chi.*
      ,pred.lwr
      ,pred.upper
...
### Plot the Results {data-height=1500}
...
```
```{r chicago_graph, echo=FALSE, warning=FALSE, message=FALSE}
...
fig <- ggplotly(p, tooltip='names', height=1500, width=1200)
fig
```

Conclusion {data-icon="ion-ios-star"}
===================================== 
![](pics/thankyou.png)
